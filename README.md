...
这是 smart-retry 在 **v1.0.2** 引入的核心升级。旧版本采用纯轮询模式，任务调度精度受限于 `taskFindInterval`（默认 20 秒），无法满足精确到秒级触发需求。
...
-- 仅对活跃任务去重；终态历史因 active_flag 为 NULL 可重复保留。
